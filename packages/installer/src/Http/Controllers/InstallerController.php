...
class InstallerController extends Controller
{
    ...
    public function step2(Request $request, $step)
    {
        try {
            ...
            // set current environment settings, otherwise Laravel doesn't recognize them
            putenv('DB_HOST=' . $request->host);
            putenv('DB_PORT=' . $request->port);
            putenv('DB_DATABASE=' . $request->name);
            putenv('DB_USERNAME=' . $request->username);
            putenv('DB_PASSWORD=' . $request->password);
